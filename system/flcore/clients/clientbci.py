...
class clientbci:
    def __init__(self, args, id, train_samples, test_samples, **kwargs):
        ...
        self.model = Ensemble(
            ...
            head_c=copy.deepcopy(self.model.head),
            feature_extractor=copy.deepcopy(self.model.feature_extractor),
            feature_extractor_c=copy.deepcopy(self.model.feature_extractor)
        )
    ...
    def set_parameters_c(self, feature_extractor):
        for new_param, old_param in zip(feature_extractor.parameters(), self.model.feature_extractor_c.parameters()):
            old_param.data = new_param.data.clone()

    def set_head_c(self, head):

        for new_param, old_param in zip(head.parameters(), self.model.head_c.parameters()):
            old_param.data = new_param.data.clone()
        ...
        for _ in range(self.local_steps):
            ...
            for i, (x, y) in enumerate(trainloader):
                if type(x) == type([]):
                    ...
                    x = x.to(self.device)
                y = y.to(self.device)
                output, rep, rec, rep_base = self.model(x, is_rep=True, context=self.context)
                loss = self.loss(output, y)
                loss += MMD(rec, rep, 'rbf', self.device) * self.lamda
...
class Ensemble(nn.Module):
    def __init__(self, model, cs, head_g, head_c, feature_extractor, feature_extractor_c) -> None:
        super().__init__()
        self.model = model
        self.head_g = head_g
        self.head_c = head_c
        self.feature_extractor = feature_extractor
        self.feature_extractor_c = feature_extractor_c

        for param in self.head_g.parameters():
            param.requires_grad = False
        for param in self.feature_extractor.parameters():
            param.requires_grad = False
        for param in self.head_c.parameters():
            param.requires_grad = False
        for param in self.feature_extractor_c.parameters():
            param.requires_grad = False
    ...
    def forward(self, x, is_rep=False, context=None):
        ...
        rec = self.feature_extractor_c(x)
        gate_in = rep
        ...
        if self.flag == 0:
            rep_p, rep_c, rep_g = self.gate(rep, self.tau, self.hard, gate_in, self.flag)
            output = self.model.head(rep_p) + self.head_g(rep_g) + self.head_c(rep_c)
        elif self.flag == 1:
            ...
            rep_c = self.gate(rep, self.tau, self.hard, gate_in, self.flag)
            output = self.head_c(rep_c)

        if is_rep:
            return output, rep, rec, self.feature_extractor(x)
        else:
            return output
...
class Gate(nn.Module):
    ...
    def forward(self, rep, tau=1, hard=False, context=None, flag=0):
        pm, cm, gm = self.cs(context, tau=tau, hard=hard)
